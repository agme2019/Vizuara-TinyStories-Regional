...
<ul>
  ...
  <li>To modify the list of nouns, verbs, etc., please modify <code>.txt</code> files at <code>prompting/prompt_gen/<i>lanauge</i></code></li>
  <li>Prompt templates/complexities can be referred to/modified through <code>create_prompts.py</code>. We compare the same in our paper</li>
  <li>Unique (sampling is unique and not random) prompts can be generated by running <code>generate_prompts.py</code></li>
  <li>Generated prompts are written to a <code>.json</code> file. <b>Sharding the file is recommended (below)</b></li>
</ul>
...
<ul>
  <li>Prompts are read from the file/shards and "sent" to the specified LLM (using <a href="https://github.com/xtekky/gpt4free">G4F)</li> 
  <li>Multithreaded API calls result in a max speed of <b>100 stories/min</b> for GPT-4o-mini and GPT-4o (occasionally).</li>
      <ul>
        ...
        <li><i>Optimal config</i>: <b>16 vCPUs</b>, running <b>4 sessions</b> concurrently (one for each shard), each with <b>16 threads</b></li>
      </ul>
