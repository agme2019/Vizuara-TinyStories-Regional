...
<ul>
  <li>Prompts are generated by sampling a unique set of noun, verb, adjective and feature</li>
  <li>In case you wish to modify the list of nouns, verbs, adjectives or features, please modify <code>.txt</code> files in <code>prompting/prompt_gen/<i>lanauge</i></code></li>
  <li>Various prompt templates/complexities can be referred to/modified through <code>create_prompts.py</code>. We compare the same in our paper</li>
  <li>Desired number of unique (sampling is unique and not random) prompts can be generated by running <code>generate_prompts.py</code></li>
  <li>All generated prompts are written to a <code>.json</code> file. <b>Sharding the file is recommended while making API calls (below)</b></li>
</ul>
...
<ul>
  <li>Prompts are read from the aforementioned file/shards and "sent" to the specified LLM (using <a href="https://github.com/xtekky/gpt4free">G4F)</li> 
  <li>Multithreaded API calls result in a max speed of <b>100 stories/min</b> for GPT-4o-mini and GPT-4o (occasionally).</li>
      <ul>
        <li>It is recommended number of threads = number of vCPUs/4</li>
        <li>Each thread writes to a common file. Once every 10% of total progress</li>
        <li><i>Optimal multithreading config</i>: <b>16 vCPUs</b>, running <b>4 sessions</b> concurrently (one for each shard), each with <b>16 threads</b></li>
      </ul>
  <li>Please look into <code>make_requests.py</code> for customizing the prompting schema</li>
  <li>Please look into <code>request_helper.py</code> for a detailed look into the process</li>
      <ul>
        <li>Various regex (data cleanup) features</li>
        <li>API/LLM is prompted until a valid story is generated for each prompt</li>
      </ul>
  <li>To start the data generation process, please run the file <code>make_requests.py</code></li>
</ul>

```sh
python make_requests.py
```
_**TIP💡**: To run data generation in the background (detached VM session)_
```sh
